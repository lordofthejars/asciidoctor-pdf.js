...
[source, java]
----
@Schema(name="Developers", description="POJO that represents a developer.")
...
// tag::udpate_8_10[]
You can also serve OpenAPI Schema from static files instead of dynamically generate from annotation scanning.
...
You can disable the scanning documents by adding next configuration property: `mp.openapi.scan.disable=true`.
