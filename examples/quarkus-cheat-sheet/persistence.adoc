...
There are three ways to create your schema and your own Marshaller:
...
`DEFAULT` credentials provider chain: System properties `aws.accessKeyId`, `aws.secretKey` -> Env. Varables `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` -> Credentials profile `~/.aws/credentials` -> Credentials through the Amazon EC2 container service if the `AWS_CONTAINER_CREDENTIALS_RELATIVE_URI` set -> Credentials through Amazon EC2 metadata service.
...
Credentials specific parameters prefixed with `quarkus.dynamodb.credentials`:
